...
/// Writes the mic + system channels (for per-channel transcription) for one
/// recording. Writers open lazily on the first window so an unconfigured
/// recording (no channel dir) is a no-op.
